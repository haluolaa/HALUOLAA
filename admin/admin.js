...
window.showSection = function(name) {
  ...
  const titles = { overview:'نظرة عامة', products:'المنتجات', 'add-product':'إضافة منتج', discounts:'الخصومات', orders:'الطلبات', settings:'الإعدادات', customize:'تخصيص الموقع' };
...
window.resetCustomize = function() {
  ...
  applySettingsToForm({
    ...
    site_title: 'HALOULA', site_subtitle: 'مصممة أزياء',
...
function renderOrders(orders) {
  ...
  if (!orders.length) {
    list.innerHTML = '<div class="orders-empty"><i class="fas fa-inbox"></i><p>No orders yet</p></div>';
    return;
  }
  ...
  list.innerHTML = orders.map(o => {
    ...
    return `
    <div class="order-card ${o.status || 'pending'}" id="order-${o.id}">
      <div class="order-head">
        <div>
          <div class="order-id">#${o.id}</div>
          <div class="order-date">${date}</div>
        </div>
        <span class="order-status status-${o.status || 'pending'}">${(o.status || 'pending').toUpperCase()}</span>
      </div>
      <div class="order-customer">
        ...
        <div class="order-customer-item"><i class="fas fa-credit-card"></i> ${o.payment_method}</div>
      </div>
      ${o.notes ? `<div class="order-notes">📝 ${o.notes}</div>` : ''}
      <div class="order-items">
        ${items.map(item => `
          <div class="order-item-card">
            <img src="${item.image || ''}" alt="${item.name}" onclick="openLightboxAdmin('${item.image}')" onerror="this.style.display='none'"/>
            <div class="order-item-info">
              <div class="order-item-name">${item.name}</div>
              <div class="order-item-meta">Size: ${item.size} × ${item.qty}</div>
              <div class="order-item-price">${item.price * item.qty} EGP</div>
            </div>
          </div>`).join('')}
      </div>
      <div class="order-footer">
        <div class="order-total">Total: <span>${(o.total || 0).toLocaleString()} EGP</span></div>
        <div class="order-actions">
          <button class="btn-wa-reply" onclick="replyOnWhatsApp(${o.id})">
            <i class="fab fa-whatsapp"></i> Reply on WhatsApp
          </button>
          <select class="btn-status" onchange="updateOrderStatus(${o.id}, this.value)">
...
window.replyOnWhatsApp = function(orderId) {
  ...
  const itemsList = items.map(i => `• ${i.name} (${i.size}) ×${i.qty} = ${i.price * i.qty} EGP`).join('\n');

  const msg = `Hello ${order.customer_name}! 👋

Thank you for your order from HALOULA ✨
...
For any questions, feel free to message us here.
— HALOULA Team 🌸`;

  const phone = order.customer_phone.replace(/[^0-9]/g, '');
  const fullPhone = phone.startsWith('0') ? '2' + phone : phone.startsWith('2') ? phone : '2' + phone;
...
window.updateOrderStatus = async function(orderId, status) {
  ...
  if (card) {
    ...
    if (badge) { badge.className = `order-status status-${status}`; badge.textContent = status.toUpperCase(); }
  }
  const order = allOrders.find(o => o.id === orderId);
