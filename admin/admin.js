...
function renderDiscounts() {
  ...
  if (!adminDiscounts.length) { list.innerHTML = '<p style="color:#888;text-align:center;padding:20px">No discounts yet</p>'; return; }
  list.innerHTML = adminDiscounts.map(d => `
    <div class="discount-card">
      <div>
        <div class="discount-code">${d.code}</div>
        <div class="discount-info">${d.expiry ? `Expires: ${d.expiry}` : 'No expiry date'}</div>
      </div>
...
window.saveSettings = async function() {
  ...
  if (!newPass) { alert('⚠️ Enter new password'); return; }
