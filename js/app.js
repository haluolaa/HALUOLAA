...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { closeLightbox(); closeModal(); closeReviewModal(); }
  });
});
...
window.renderPaymentMethods = function() {
  ...
  if (s.instapay_enabled && s.instapay_link) {
    const account = s.instapay_account || s.instapay_link;
    html += `
    <div class="pay-card">
      <a href="${s.instapay_link}" target="_blank" class="pay-img-btn">
        <img src="https://i.ibb.co/gFtK0qQB/insta.png" alt="InstaPay"/>
        <span class="pay-img-label"><i class="fas fa-external-link-alt"></i> افتح InstaPay</span>
      </a>
      ${account ? `
      <div class="pay-card-info">
        <div class="pay-info-row">
          <span>الحساب</span>
          <span class="pay-val">${account}</span>
        </div>
      </div>
      <div class="pay-card-btns">
        <button class="pay-action-btn" onclick="copyText('${account}')">
          <i class="fas fa-copy"></i> نسخ الحساب
        </button>
      </div>` : ''}
    </div>`;
  }
  ...
  // Vodafone Cash
  if (s.vodafone_enabled && s.vodafone_number) {
    html += `
    <div class="pay-card">
      <a href="vodafonecash://send?msisdn=${s.vodafone_number}" class="pay-img-btn" onclick="openVFCash(event,'${s.vodafone_number}')">
        <img src="https://i.ibb.co/0y6yKR65/image.png" alt="Vodafone Cash"/>
        <span class="pay-img-label"><i class="fas fa-mobile-alt"></i> افتح Vodafone Cash</span>
      </a>
      <div class="pay-card-info">
        <div class="pay-info-row">
          <span>الرقم</span>
          <span class="pay-val">${s.vodafone_number}</span>
        </div>
      </div>
      <div class="pay-card-btns">
        <button class="pay-action-btn" onclick="copyText('${s.vodafone_number}')">
  ...
  if (!html) {
    html = `<p style="text-align:center;color:var(--muted);padding:20px 0;font-size:14px">
      لا توجد طرق دفع متاحة حالياً — تواصل معنا على Instagram
    </p>`;
  }
...
window.openVFCash = function(e, number) {
  e.preventDefault();
  const num = number || '01021102607';
  const deepLink = `vodafonecash://send?msisdn=${num}`;
...
window.confirmPaid = async function() {
  ...
  try {
    ...
    const itemsList = cart.map(i => `• ${i.name_en||i.name} (${i.size}) ×${i.qty} = ${i.price*i.qty} EGP`).join('\n');
    const msg = `🛍️ *طلب جديد - HALOULAA*\n\n👤 الاسم: ${name}\n📞 الهاتف: ${phone}\n📍 العنوان: ${address}\n\n${itemsList}\n\n💰 *الإجمالي: ${total.toLocaleString()} EGP*\n\n✅ العميل أكد الدفع`;
    window.open(`https://wa.me/201021102607?text=${encodeURIComponent(msg)}`, '_blank');
  } catch(e) {}
  ...
  document.querySelector('.order-modal').innerHTML = `
    <div style="text-align:center;padding:20px 10px">
      ...
      <p style="color:#f0e6d3;font-size:15px;line-height:1.7;margin-bottom:8px">Thank you! Your order has been received successfully.</p>
      <p style="color:#a89880;font-size:14px;line-height:1.7;margin-bottom:24px">We will review your payment and contact you soon. 📦</p>
