...
function renderProducts(list, page = 1) {
  ...
  grid.innerHTML = pageItems.map(p => {
    ...
    const productName = (typeof currentLang !== 'undefined' && currentLang === 'en' && p.name_en) ? p.name_en : p.name;
    ...
    const stockText = stock <= 3 ? `🔥 آخر ${stock} قطع!` : `✅ متاح (${stock} قطعة)`;
    ...
    <div class="product-card" data-id="${p.id}">
      ...
      <div class="product-info">
        ...
        <div class="product-views"><i class="fas fa-eye"></i> ${views} شخص يشاهد الآن</div>
...
// ===== FAKE REVIEWS =====
const FAKE_REVIEWS = [
  { name: 'سارة أحمد', avatar: 'س', stars: 5, text: 'الفستان جميل جداً والخامة ممتازة! وصل بسرعة وكان مطابق للصورة تماماً. هطلب تاني قريباً 🌸', product: 'فستان شيفون وردي', date: 'منذ يومين' },
  { name: 'نور محمد', avatar: 'ن', stars: 5, text: 'أحسن موقع اتعاملت معاه! الأسعار مناسبة جداً والجودة عالية. البنطلون مريح ومناسب للمقاس', product: 'بنطلون جينز كلاسيك', date: 'منذ أسبوع' },
  { name: 'ريم علي', avatar: 'ر', stars: 5, text: 'الطقم حلو أوي والألوان زي ما في الصورة بالظبط. التوصيل كان سريع جداً. شكراً HALOULA ❤️', product: 'طقم كاجوال أنيق', date: 'منذ 3 أيام' },
  { name: 'منى حسن', avatar: 'م', stars: 4, text: 'البلوزة ناعمة ومريحة جداً. المقاس كان صح. بس التوصيل اتأخر شوية. بس المنتج تمام 👍', product: 'بلوزة كاجوال ناعمة', date: 'منذ أسبوعين' },
  { name: 'دينا خالد', avatar: 'د', stars: 5, text: 'فستان الأطفال جميل جداً وبنتي بتحبه أوي! الخامة ناعمة ومريحة على جلدها. هطلب أكتر 🎀', product: 'فستان أطفال ملون', date: 'منذ 5 أيام' },
  { name: 'هبة سامي', avatar: 'ه', stars: 5, text: 'التيشيرت أوفرسايز تماماً زي ما كنت عايزاه! الجودة ممتازة والسعر معقول جداً. موصي بيه 💕', product: 'تيشيرت بيزيك أوفرسايز', date: 'منذ يوم' },
];

function renderReviews() {
  const grid = document.getElementById('reviewsGrid');
  if (!grid) return;
  grid.innerHTML = FAKE_REVIEWS.map(r => `
    <div class="review-card">
      ...
      <div class="review-verified"><i class="fas fa-check-circle"></i> عميلة موثقة</div>
    </div>
  `).join('');
}

// ===== FAKE STOCK & VIEWS =====
function getFakeStock(productId) {
  const stocks = [3, 5, 7, 2, 8, 4, 6, 1, 9, 3, 5, 7];
  return stocks[productId % stocks.length] || 5;
}

function getFakeViews(productId) {
  const views = [12, 8, 23, 5, 17, 31, 9, 14, 6, 19, 11, 25];
  return views[productId % views.length] || 10;
}

function getFakeRating(productId) {
  const ratings = [4.9, 4.8, 5.0, 4.7, 4.9, 4.8, 5.0, 4.6, 4.9, 4.8, 5.0, 4.7];
  const reviews = [124, 89, 203, 67, 156, 312, 94, 178, 45, 231, 88, 167];
  return { rating: ratings[productId % ratings.length], count: reviews[productId % reviews.length] };
}
