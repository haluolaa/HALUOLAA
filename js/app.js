...
async function loadCategories() {
  try {
    ...
    // Update category select in admin product form
    updateCategorySelect(data);
  } catch(e) {}
}

function updateCategorySelect(cats) {
  // Update filterCat titles map
  window._catTitles = { all: 'All Products' };
  cats.forEach(c => { window._catTitles[c.name_ar] = c.name_en; });
}
...
window.renderPaymentMethods = function() {
  ...
  if (s.vodafone_enabled && (s.vodafone_link || s.vodafone_number)) {
    ...
    html += `
    <div class="pay-card">
      <a href="${href}" ${isDeepLink ? `onclick="handleVFCash(event,'${s.vodafone_number}')"` : 'target="_blank"'} class="pay-img-btn">
  ...
  if (!html) {
    html = `<p style="text-align:center;color:var(--muted);padding:20px 0;font-size:14px">
      لا توجد طرق دفع متاحة — تواصل معنا على
...
window.confirmPaid = async function() {
  ...
  modal.innerHTML = `
    <div style="text-align:center;padding:10px 0 20px">
      ...
      <h3 style="color:var(--gold);font-size:20px;margin-bottom:8px">تم استلام طلبك!</h3>
      <p style="color:#f0e6d3;font-size:14px;line-height:1.7;margin-bottom:20px">سنتواصل معك على الإنستجرام لتأكيد الدفع والشحن 📦</p>
      <div style="background:rgba(201,169,110,.08);border:1px solid rgba(201,169,110,.2);border-radius:12px;padding:14px;margin-bottom:20px">
        <p style="color:var(--gold);font-size:13px;font-weight:600">📲 تواصل معنا على الإنستجرام لإتمام الطلب</p>
        <a href="https://ig.me/m/haloula_designer" target="_blank" class="btn btn-primary w-full" style="margin-top:10px;display:flex;align-items:center;justify-content:center;gap:8px;text-decoration:none">
          <i class="fab fa-instagram"></i> تواصل على Instagram
        </a>
      </div>
      <p style="color:var(--muted);font-size:13px;margin-bottom:14px">أو ادفع الآن وأرسل إيصال الدفع على الإنستجرام:</p>
      <div id="confirmPayMethods"></div>
      <button class="btn btn-outline w-full" onclick="closeModal()" style="margin-top:16px">متابعة التسوق</button>
    </div>`;
