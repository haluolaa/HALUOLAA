...
const DEFAULT_CATS_FALLBACK = [
  { name_ar:'فساتين', name_en:'Dresses', icon:'fas fa-female' },
  { name_ar:'بلوزات', name_en:'Tops', icon:'fas fa-tshirt' },
  { name_ar:'بناطيل', name_en:'Pants', icon:'fas fa-stream' },
  { name_ar:'أطقم',   name_en:'Sets', icon:'fas fa-layer-group' },
  { name_ar:'تيشيرتات', name_en:'T-Shirts', icon:'fas fa-tshirt' },
  { name_ar:'جيبات',  name_en:'Skirts', icon:'fas fa-circle' },
  { name_ar:'أطفال',  name_en:'Kids', icon:'fas fa-child' },
];

async function loadCategories() {
  try {
    const { data } = await supabase.from('categories').select('*').neq('is_active', false).order('sort_order');
    const cats = (data && data.length) ? data : DEFAULT_CATS_FALLBACK;
    ...
    cats.forEach(cat => {
      ...
      li.innerHTML = `<i class="${cat.icon || 'fas fa-tag'}"></i> ${displayName}`;
      list.appendChild(li);
    });
    ...
    list.innerHTML = '';
    if (allItem) list.appendChild(allItem);
    DEFAULT_CATS_FALLBACK.forEach(cat => {
      const li = document.createElement('li');
      li.dataset.cat = cat.name_ar;
      li.onclick = function() { filterCat(this, cat.name_ar); };
      li.innerHTML = `<i class="${cat.icon}"></i> ${cat.name_en}`;
      list.appendChild(li);
    });
    updateCategorySelect(DEFAULT_CATS_FALLBACK);
  }
}
          ...
          <div class="price-block">
            ${p.old_price && p.old_price > p.price ? `
              <div class="price-top-row">
                ...
                <span class="discount-badge">-${Math.round((1 - p.price/p.old_price)*100)}%</span>
              </div>
              <div class="product-old-price">${p.old_price} EGP</div>
            ` : `
...
window.applyDiscount = async function() {
  const code = document.getElementById('custDiscount').value.trim().toUpperCase();
  const msgEl = document.getElementById('discountMsg');
  if (!code) { 
    msgEl.style.display = 'block';
    msgEl.style.background = 'rgba(231,76,60,.1)';
    msgEl.style.color = '#e74c3c';
    msgEl.textContent = '⚠️ Please enter a discount code';
    return; 
  }

  msgEl.style.display = 'block';
  msgEl.style.background = 'rgba(201,169,110,.08)';
  ...
  try {
    ...
    const discount = data && data.length > 0 ? data[0] : null;
    ...
    if (!discount.is_active) {
      msgEl.style.background = 'rgba(231,76,60,.1)';
      msgEl.style.color = '#e74c3c';
      msgEl.textContent = '❌ This code is no longer active';
      appliedDiscount = null;
      updateOrderSummaryWithDiscount();
      return;
    }
    ...
    if (discount.expiry && new Date(discount.expiry) < new Date()) {
      msgEl.style.background = 'rgba(231,76,60,.1)';
      msgEl.style.color = '#e74c3c';
      msgEl.textContent = '❌ This code has expired';
      appliedDiscount = null;
      updateOrderSummaryWithDiscount();
      return;
    }
    ...
    msgEl.style.background = 'rgba(46,204,113,.1)';
    msgEl.style.color = '#2ecc71';
    msgEl.textContent = `✅ Code applied! ${discount.percent}% discount`;
    updateOrderSummaryWithDiscount();

  } catch(e) {
    msgEl.style.background = 'rgba(231,76,60,.1)';
    msgEl.style.color = '#e74c3c';
    msgEl.textContent = '❌ Error checking code, try again';
    appliedDiscount = null;
  }
}
...
function updateOrderSummaryWithDiscount() {
  ...
  const discountRow = discount > 0
    ? `<div class="s-item" style="color:#2ecc71"><span>🏷️ Discount (${appliedDiscount.percent}%)</span><span>-${discount} EGP</span></div>`
    : '';
...
window.checkout = function() {
  ...
  // reset discount UI
  const discInput = document.getElementById('custDiscount');
...
window.confirmPaid = async function() {
  ...
  modal.innerHTML = `
    <div style="text-align:center;padding:10px 0 20px">
      ...
      <p style="color:#f0e6d3;font-size:14px;line-height:1.7;margin-bottom:20px">We'll contact you to confirm payment & shipping 📦</p>
      <a href="https://ig.me/m/haloula_designer" target="_blank" class="btn btn-primary w-full"
        style="display:flex;align-items:center;justify-content:center;gap:8px;text-decoration:none;margin-bottom:12px">
        <i class="fab fa-instagram"></i> Contact on Instagram
      </a>
      <button class="btn btn-outline w-full" onclick="closeModal()">Continue Shopping</button>
    </div>`;
